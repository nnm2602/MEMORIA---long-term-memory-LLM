...
# create_knowledge_tree(5,15)
#we'll pick up from layer 3 
# we have a problem -> layer one is incomplete 

# zeroth_layer = json_to_nodes("./knowledge_tree_concurrent/layer_0.json")
# first_layer = json_to_nodes("./knowledge_tree_concurrent/layer_1.json")
# print(len(zeroth_layer))
# print(len(first_layer))
# print(len(zeroth_layer)==len(first_layer))


# zeroth_layer = json_to_nodes("file_nodes.json")
# first_layer = json_to_nodes("first_layer_nodes.json")
# print(len(zeroth_layer))
# print(len(first_layer))
# print(len(zeroth_layer) == len(first_layer))
...
# it's not letting me do my thing okay so how do i do this, maybe do a half ass way and limit that thing to thirty or something per shit. 
# we have to change this for the two functions so why not create a separate function to do everything in 
'''
So like what the fuck do we do now? 
- we need to have a log function
- we need to fix the bug that is causing layer one to only summarize the first batch 
+ let's just fix the bug first 

+ full test works so getting the thing directly from the json file works
+ so it's like encryption and decryption thing then 
+ okay so my theory is that it didn't save layer one because it hasn't had the time to "recursively retreat". In other words, it saves backward ( the last layer first then the first layer last)

+ ok so how to fix it? breakage? so like recursively whatever but if at some point the program breaks 

+ there's another fucking problem 
'''
